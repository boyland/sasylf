...
connection.onDefinition((params) => {
    ...
    if (word.includes(".")) {
        return search(word, compUnit, params.textDocument.uri, connection);
    }
...
connection.onCompletion((params, _) => {
    ...
    const word = text.slice(wordStart, offset - 1).trim().split(".")[0];
    let ast: null | ast = null;

    for (const module of compUnit.modules) {
        if (module.name.split(":")[0] == word) { console.log(module); ast = module.ast; }
    }

    if (!ast) return CompletionList.create([], true);

    const result: CompletionItem[] = [];
    let item: CompletionItem;

    for (const theorem of ast.theorems) {
        item = CompletionItem.create(theorem.name);
        item.kind = theorem.kind == "theorem" ? CompletionItemKind.Class : CompletionItemKind.Struct;
        result.push(item);
    }

    for (const judgment of ast.judgments) {
        for (const rule of judgment.rules) {
            item = CompletionItem.create(rule.name);
