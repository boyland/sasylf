...
// Hashmap mapping index of diagnostic to the diagnostic and its quickfix info
...
connection.onCompletion((params, _) => {
    ...
    // Finds the word underneath the cursor
    while (!badCharacters.includes(text[wordStart])) {
        if (!badCharacters.includes(text[wordStart])) --wordStart;
    }

    const word = text
        .slice(wordStart, offset - 1)
        .trim()
        .split(".")[0];
...
connection.onDocumentSymbol((params) => {
    ...
    if (!syntaxDeclarations) {
        logErrorToFile(
            "Syntax_declarations is not defined",
...
connection.onRequest("custom/getAST", () => compUnit);
