	...
	protected void createPackageReplaceChange(IDocument doc, TextFileChange change, RefactoringStatus status, String oldPackage) 
		...
		if (cu != null) {
			...
			if (pd.getPieces().length > 0) {
				...
				String text = doc.getCloneFor(packageLoc.getOffset(), packageLoc.getLength());
		...
		if (packageLoc != null) {
			...
			if (doc.getCloneFor(offset, length).equals(newPackage)) {
				// no change needed
				return;
			}
	...
	private void createPackageReplaceChangeModule(IFile file, IDocument doc, 
		...
		if (cu != null) {
			...
			for (int i = qualNames.size() - 1; i >= 0; --i) {
				...
				System.out.println("Replacing " + doc.getCloneFor(offset, length) + " with " + newPackage);
