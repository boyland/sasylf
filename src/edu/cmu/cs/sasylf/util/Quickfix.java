...
public class Quickfix {
  ...
  public HashMap<String, Object> makeQuickfix(VSDocument doc, VSMarker marker) {
    ...
    String extraIndent = "";
    int ind = doc.substring(lineInfo.getOffset()).indexOf(split[0]);
    VSRegion old;

    if (ind == -1) {
      old = null;
    } else {
      old = new VSRegion(ind, split[0].length());
    }
    ...
    case MISSING_CASE:
      ...
      if (lineText.contains("by contradiction on") &&
          !lineText.contains(
              "by case analysis on")) { // XXX: Could be confused by a comment
        ...
        if (l > 3 && parts[l - 2].equals("on")) {
          ...
          // System.out.println("Converting starting at column " + lo + " using
          // '" + newText + "'");
          proposals.add(new MyCompletionProposal(
              res, newText, offset, reg.getLength() - lo, newCursor, null,
              "convert to case analysis with missing case(s)", null, fixInfo));
        }
      } else {
        newText = sb.toString();
        proposals.add(new MyCompletionProposal(
