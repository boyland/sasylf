...
public class DerivationPlaceholder extends Derivation {
  ...
  public DerivationPlaceholder(String n, UpdatableErrorReport rep) {
    super(n, rep.getSpan().getLocation(),
          AndClauseUse.makeEmptyAndClause(rep.getSpan().getLocation()));
    variable = FreeVar.fresh(n, Constant.UNKNOWN_TYPE);
    report = rep;
  }

  public UpdatableErrorReport getReport() { return report; }
   ...
   */
  public void setPlaceholder(Context ctx, Substitution sub,
                             Derivation foundIn) {
    if (!sub.getDomain().contains(variable))
      return;
  ...
  public TermPrinter setTerm(Context ctx, Term t, Derivation foundIn) {
    ...
    TermPrinter tp = new TermPrinter(ctx, ctx.assumedContext,
                                     report.getSpan().getEndLocation());
